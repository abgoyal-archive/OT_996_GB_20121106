...
/*--------------------------------------------------------------------------------------*/
/*    Comments :                                                             */
/*    File     : hardware/broadcom/bt/adaptation/dtun/dtunc_bz4/dtun_hcid.c  */
/*    Labels   :                                                             */
/*======================================================================================*/
...
/*---------|------------------|--------------------------|------------------------------*/
/*11/24/2011 | Hualan.Su  | PR182297-Hualan.Su-001  |Device name displayed not normally.  */
/*---------|------------------|--------------------------|------------------------------*/
/*---------|------------------|--------------------------|------------------------------*/
/*======================================================================================*/
#include <stdio.h>
...
#define DTUN_NUM_UUIDS_IN_TABLE  28
...
void dtun_dm_sig_local_info(tDTUN_DEVICE_SIGNAL *msg)
{
   ...
   /* the Customer needs to modify this to put the customized phone name */
    if( retval < 0 )
    //CHG-s by Su Hualan: [Feature-BRANDY 2.3_CR116575]Modify for set Bluetooth device name
        //strcpy( str, DTUN_DEFAULT_DEV_NAME );
    {
        memset(str, 0, DTUN_MAX_DEV_NAME_LEN);
        property_get("ro.product.model", str, "");
    }
    //CHG-e by Su Hualan: [Feature-BRANDY 2.3_CR116575]Modify for set Bluetooth device name
...
static inline void copy_device_name(char* dest, const char* src)
{
	...
	//PR182297-Hualan.Su-001-Add-begin
	while (!bValidUtf8)
	{
		len--;
		if (len == 0)
		...
		bValidUtf8 = utf8_validate(src, 0, len);
	}
	//PR182297-Hualan.Su-001-Add-end
...
void dtun_dm_sig_device_found(tDTUN_DEVICE_SIGNAL *msg)
{
    ...
    hcid_dbus_inquiry_result(&sba, &msg->device_found.info.bd,
            msg->device_found.info.cod, msg->device_found.info.rssi, 
...
// CK - OOB support for Gingerbread
    dtun_sig_dm_local_oob_keys,
// CK - End - OOB support for Gingerbread
    /* AV signals */
    dtun_am_sig_av_event,
    /* Dual HF signal */
