...
void HTMLLinkElement::setCSSStyleSheet(const String& href, const KURL& baseURL, const String& charset, const CachedCSSStyleSheet* sheet)
{
    ...
    /*add for telecom website combobox display messy, this is temp policy*/
    String l_url = baseURL.string();
    if(l_url.contains("www.telekom.com",false) && href.contains("telekom.css",false)){
        if(sheetText.contains(".ModulL1 select",false))
        {
            sheetText.replace(String(".ModulL1 select {color:#666666;float:left;font-size:0.84em;height:1.7em;width:14.1em}"),
				           String(".ModulL1 select {color:#666666;float:left;height:3em;width:14.1em}"));
	 }
    }
