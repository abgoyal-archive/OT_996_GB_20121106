...
/*---------|------------------|--------------------------|------------------------------*/
/*11/28/2011 | Hualan.Su  | PR191761-Hualan.Su-001  |device_remove_bonding when pair error  */
/*---------|------------------|--------------------------|------------------------------*/
/*05/05/2012 | Hualan.Su  | PR259569-Hualan.Su-002  |Although, btld don't care about this reply.
We still need reply it as success immediately to cleanup the reply recorder in dbus daemon  */
/*---------|------------------|--------------------------|------------------------------*/
/*======================================================================================*/
#ifdef HAVE_CONFIG_H
#include <config.h>
...
//PR259569-Hualan.Su-002-modify begin
#ifdef BT_ALT_STACK
       debug("replying DiscoverServices as success");
       boolean success = 1;  //<<< added
       DBusMessage* reply = dbus_message_new_method_return(msg);  //<<<added
       dbus_message_append_args(reply, DBUS_TYPE_BOOLEAN, &success, DBUS_TYPE_INVALID); //<<<added
       return reply; //<<<added
#else
       return NULL;
#endif
//PR259569-Hualan.Su-002-modify end
fail:
	return g_dbus_create_error(msg, ERROR_INTERFACE ".Failed",
					"Discovery Failed");
}
...
void device_bonding_complete(struct btd_device *device, uint8_t status)
{
	...
	if (status) {
        ...
        //PR191761-Hualan.Su-001-Add begin
        #ifdef BT_ALT_STACK
              /* brcm fix CSP474493 */
              device_remove_bonding(device);
        #endif
        //PR191761-Hualan.Su-001-Add end
		return;
	}
