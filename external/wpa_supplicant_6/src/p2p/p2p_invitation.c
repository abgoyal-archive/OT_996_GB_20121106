...
{
	struct p2p_device *sa_dev = NULL;
	struct p2p_device *go_dev = NULL;
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		"P2P: Invitation Response TX callback: success=%d", success);
	p2p->cfg->send_action_done(p2p->cfg->cb_ctx);

	if (success && p2p->cfg->invitation_received) {
		sa_dev = p2p_get_device(p2p, p2p->inv_sa);
		go_dev = p2p_get_device(p2p, p2p->inv_go_dev_addr);
		...
		p2p->cfg->invitation_received(p2p->cfg->cb_ctx,
					      ...
					      p2p->inv_status,
					      p2p->inv_op_freq, 
					      sa_dev?sa_dev->info.device_name:NULL, 
					      go_dev?go_dev->info.device_name:NULL);
	}
}
