...
/*-----------|------------------|--------------------------|-----------------*/
/*2012/02/01 | Wei.YING         | Modify for PR220877      |                 */
/*-----------|------------------|--------------------------|-----------------*/
/*===========================================================================*/
...
struct p2p_group_member {
	...
	char device_name[33]; /* added by Xinrui */
...
static int p2p_client_info(struct wpabuf *ie, struct p2p_group_member *m, int offset)
{
	int len;
	if (m->client_info == NULL)
		return 0;
	len = (wpabuf_tailroom(ie) < wpabuf_len(m->client_info) - offset) ?
	      wpabuf_tailroom(ie) : wpabuf_len(m->client_info) - offset;
	wpabuf_put_data(ie, wpabuf_head(m->client_info) + offset, len);
	return wpabuf_len(m->client_info) - offset - len;
/*
...
static struct wpabuf * p2p_group_build_probe_resp_ie(struct p2p_group *group)
{
	...
	u8 *len;
	struct wpabuf *first_ie, *concat_ie = NULL;
	int group_info_len, offset, ret;
	...
	for (m = group->members; m; m = m->next) {
		offset = 0;
		while ((ret = p2p_client_info(ie, m, offset)) != 0) {
			group_info_len = group_info_len + wpabuf_len(m->client_info) - offset - ret;
			p2p_buf_update_ie_hdr(ie, len);
			if (ie != first_ie) {
				concat_ie = wpabuf_concat(concat_ie, ie);
			}
			ie = wpabuf_alloc(256);
			len = p2p_buf_add_ie_hdr(ie);
			offset = wpabuf_len(m->client_info) - ret;
		}
		if (m->client_info != NULL) {
			group_info_len = group_info_len + wpabuf_len(m->client_info) - offset - ret;
		}
	}
	WPA_PUT_LE16(group_info + 1, group_info_len);

	p2p_buf_update_ie_hdr(ie, len);

	if (ie != first_ie) {
		concat_ie = wpabuf_concat(concat_ie, ie);
		ie = wpabuf_concat(first_ie, concat_ie);
	}
	return ie;
}
			  ...
			  const u8 *ie, size_t len)
{
	...
	if (m) {
		wpa_msg(group->p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Existing "
			"client " MACSTR " in group; num_members=%u/%u",
	...
	/* save device name */
	if ((m->p2p_ie != NULL) && (p2p_parse_p2p_ie(m->p2p_ie, &msg) == 0)) {
		if(msg.device_name[0] != '\0') {
			strcpy(m->device_name, msg.device_name);
			wpa_printf(MSG_ERROR, "device_namee =%s\n", msg.device_name);
		}
	}
	...
	wpa_msg(group->p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Add client " MACSTR
		" to group (p2p=%d client_info=%d); num_members=%u/%u, device_name=%s",
		MAC2STR(addr), m->p2p_ie ? 1 : 0, m->client_info ? 1 : 0,
		group->num_members, group->cfg->max_clients, m->device_name);
	if (group->num_members == group->cfg->max_clients)
		group->beacon_update = 1;
	p2p_group_update_ies(group);
...
int p2p_group_get_clients( void *wpa_s, struct p2p_group *group )
{
    struct p2p_group_member *m;
    ...
    for (m = group->members; m; m = m->next) {
        wpa_printf(MSG_DEBUG, "get client member\n");

        //PR220877-Wei.YING-001 MOD Begin
        if (m->p2p_ie){
            os_memset(&msg, 0, sizeof(msg));
            p2p_parse_p2p_ie(m->p2p_ie, &msg);
            wpa_msg(wpa_s,  MSG_INFO, "P2P-DEVICE-FOUND " MACSTR
            " p2p_dev_addr=" MACSTR
            " pri_dev_type=%s"
            " name='%s' config_methods=0x%x "
            "dev_capab=0x%x group_capab=0x%x",
            MAC2STR(m->addr), MAC2STR(m->dev_addr),
            wps_dev_type_bin2str(msg.pri_dev_type, devtype,
            sizeof(devtype)),
            m->device_name, msg.config_methods,
            m->dev_capab , 0);
        }
        //PR220877-Wei.YING-001 MOD End
						      ...
						      const u8 *dev_id)
{
	...
	for (m = group->members; m; m = m->next) {
		if (os_memcmp(zero, m->dev_addr, ETH_ALEN) == 0) {
			if (os_memcmp(dev_id, m->addr, ETH_ALEN) == 0)
				return m;
		} else {
...
static struct p2p_group_member * p2p_group_get_client_by_interface_addr(struct p2p_group *group,
...
const u8 * p2p_group_get_client_device_addr(struct p2p_group *group,
						      const u8 *interface_addr)
{
	struct p2p_group_member *m;
	
	m = p2p_group_get_client_by_interface_addr(group, interface_addr);
	if (m != NULL) {
		return m->dev_addr;
	}
...
int p2p_get_client_info(struct p2p_group *group, const u8 *addr, int next,
		      char *buf, size_t buflen)
{
	...
	if (addr) {
		member = p2p_group_get_client(group, addr);
	} else {
	...
	if (os_memcmp(zero, member->dev_addr, ETH_ALEN) == 0) {
		res = os_snprintf(pos, end - pos, MACSTR "\n",
				  MAC2STR(member->addr));
	} else {
		res = os_snprintf(pos, end - pos, MACSTR "\n",
				  MAC2STR(member->dev_addr));
	}
