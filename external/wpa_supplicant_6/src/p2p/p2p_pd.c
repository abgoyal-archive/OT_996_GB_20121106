...
void p2p_prov_disc_req_wait_rx_resp_timeout(void *eloop_ctx, void *timeout_ctx);
				...
				const u8 *data, size_t len)
{
    ...
    //Yingwei -Add-S
    if(msg.wps_config_methods == 0 && req_time < 20){
		req_time ++;
        p2p_parse_free(&msg);
	/* Added by Xinrui for multi probe test */
	eloop_cancel_timeout(p2p_prov_disc_req_wait_rx_resp_timeout, p2p, NULL);
	eloop_register_timeout(1, 0, p2p_prov_disc_req_wait_rx_resp_timeout, p2p, NULL);
        return;
    }else{
		req_time = 0;
	}
			   ...
			   int join)
{
	struct wpabuf *req;
	int freq;
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "[DEBUG] p2p_send_prov_disc_req : Send Provision Discovery Request");
...
void p2p_prov_disc_req_wait_rx_resp_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "[DEBUG] p2p_prov_disc_req_wait_rx_resp_timeout :  p2p_send_prov_disc_req again");
	p2p_send_prov_disc_req(p2p, p2p->go_neg_peer, prov_disc_join_flags);
}
...
//ZZZDEBUG
	prov_disc_join_flags = join;
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		"[DEBUG] Cancel the p2p_prov_disc_req_wait_rx_resp_timeout timer");
	eloop_cancel_timeout(p2p_prov_disc_req_wait_rx_resp_timeout, p2p, NULL);
	eloop_register_timeout(1, 0, p2p_prov_disc_req_wait_rx_resp_timeout, p2p, NULL);
