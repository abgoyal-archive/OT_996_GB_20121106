...
int p2p_connect_send(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	int freq;
		...
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
			"[DEBUG] p2p_connect_send : Sending GO Negotiation Request");
...
#ifdef ANDROID
		p2p->cfg->go_neg_res_rx(p2p->cfg->cb_ctx, sa,
									*msg.status);
#endif
		if (*msg.status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE || *msg.status == P2P_SC_FAIL_REJECTED_BY_USER) {
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
