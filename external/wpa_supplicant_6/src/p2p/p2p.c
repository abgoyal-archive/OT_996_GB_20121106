...
static void p2p_sd_state_timeout(void *eloop_ctx, void *timeout_ctx);
...
extern void p2p_prov_disc_req_wait_rx_resp_timeout(void *eloop_ctx, void *timeout_ctx);
...
static void p2p_expire_peers(struct p2p_data *p2p)
{
	...
	dl_list_for_each_safe(dev, n, &p2p->devices, struct p2p_device, list) {
		...
		for (g = 0; g < p2p->num_groups; g++) {
			if (p2p_group_get_client_interface_addr(p2p->groups[g], dev->info.p2p_device_addr) != NULL) {
				wpa_printf(MSG_DEBUG, "P2P: Peer entry "
					"p2p_dev_addr=" MACSTR " in group",
...
#ifdef ANDROID
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, P2P_EVENT_DEVICE_EXPIRED
			"p2p_dev_addr=" MACSTR,
			MAC2STR(dev->info.p2p_device_addr));
		wpa_printf(MSG_DEBUG, P2P_EVENT_DEVICE_EXPIRED
			"p2p_dev_addr=" MACSTR,
			MAC2STR(dev->info.p2p_device_addr));
#endif
...
static const char *p2p_sd_state_txt(int state)
{
	switch (state) {
	/*p2p Serv Disc*/
	case SD_IDLE:
...
void p2p_set_sd_state(struct p2p_data *p2p, int new_state)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: SD_State %s -> %s",
		p2p_sd_state_txt(p2p->sd_state), p2p_sd_state_txt(new_state));
	p2p->sd_state = new_state;
}
...
void p2p_sd_set_timeout(struct p2p_data *p2p, unsigned int sec, unsigned int usec)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		"p2p_sd_set_timeout : Set timeout (state=%s): %u.%06u sec",
		p2p_sd_state_txt(p2p->sd_state), sec, usec);
	eloop_cancel_timeout(p2p_sd_state_timeout, p2p, NULL);
	eloop_register_timeout(sec, usec, p2p_sd_state_timeout, p2p, NULL);
}
...
void p2p_sd_clear_timeout(struct p2p_data *p2p)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "p2p_sd_clear_timeout : Clear timeout (state=%s)",
		p2p_sd_state_txt(p2p->sd_state));
	eloop_cancel_timeout(p2p_sd_state_timeout, p2p, NULL);
}
				 ...
				 const u8 *gi, size_t gi_len)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		wpa_printf(MSG_DEBUG,
			"%s(): %s dev->info.p2p_device_addr= " MACSTR " dev->member_in_go_iface= " MACSTR, __func__, dev->info.device_name, MAC2STR(dev->info.p2p_device_addr), MAC2STR(dev->member_in_go_iface));
		if (os_memcmp(dev->member_in_go_iface, go_interface_addr,
			      ETH_ALEN) == 0) {
			wpa_printf(MSG_DEBUG,
				"%s(): %s clear member_in_go_iface and member_in_go_dev", __func__, dev->info.device_name);
	...
	for (c = 0; c < info.num_clients; c++) {
		...
		wpa_printf(MSG_DEBUG,
			"%s(): cli->p2p_device_addr= " MACSTR, __func__, MAC2STR(cli->p2p_device_addr));
		if (dev) {
			/*
...
#if 0 //def ANDROID_BRCM_P2P_PATCH
		   ...
		   const u8 *ies, size_t ies_len)
{
	...
	if (msg.capability) {
		wpa_printf(MSG_DEBUG,
			"%s(): %s p2p_dev_addr=" MACSTR " addr=" MACSTR " dev->info.group_capab= 0x%x msg.capability[1]= 0x%x",
			__func__, dev->info.device_name, MAC2STR(p2p_dev_addr), MAC2STR(addr), dev->info.group_capab, msg.capability[1]);
		if ( ((dev->info.group_capab & P2P_GROUP_CAPAB_GROUP_OWNER) == P2P_GROUP_CAPAB_GROUP_OWNER) &&
			 ((msg.capability[1] & P2P_GROUP_CAPAB_GROUP_OWNER) != P2P_GROUP_CAPAB_GROUP_OWNER) ) {
			/*
			 * Clear old data for this group because the device is not group owner anymore.
			 */
			wpa_printf(MSG_DEBUG,
				"%s(): %s p2p_dev_addr=" MACSTR " addr=" MACSTR " not group owner anymore",
				__func__, dev->info.device_name, MAC2STR(p2p_dev_addr), MAC2STR(addr));
			struct p2p_device *client;
			dl_list_for_each(client, &p2p->devices, struct p2p_device, list) {
				wpa_printf(MSG_DEBUG,
					"%s(): %s " MACSTR " member_in_go_iface=" MACSTR " member_in_go_dev=" MACSTR,
					__func__, client->info.device_name, MAC2STR(client->info.p2p_device_addr), MAC2STR(client->member_in_go_iface), MAC2STR(client->member_in_go_dev));
				if (os_memcmp(client->member_in_go_dev, p2p_dev_addr, ETH_ALEN) == 0) {
					wpa_printf(MSG_DEBUG,
						"%s(): %s " MACSTR " clear member_in_go_iface=" MACSTR " member_in_go_dev=" MACSTR,
						__func__, client->info.device_name, MAC2STR(client->info.p2p_device_addr), MAC2STR(client->member_in_go_iface), MAC2STR(client->member_in_go_dev));
					os_memset(client->member_in_go_iface, 0, ETH_ALEN);
					os_memset(client->member_in_go_dev, 0, ETH_ALEN);
				}
			}
		}
	}
	...
	p2p->cfg->dev_found(p2p->cfg->cb_ctx, addr, &dev->info,
			    !(dev->flags & P2P_DEV_REPORTED_ONCE), NULL);
	dev->flags |= P2P_DEV_REPORTED | P2P_DEV_REPORTED_ONCE;
...
static void p2p_search(struct p2p_data *p2p)
{
	...
	if (p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, type, freq,
			       p2p->num_req_dev_types, p2p->req_dev_types, NULL, 0) < 0)
	{
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
...
// ZZZ
static void p2p_neg_req_wait_rx_resp_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct p2p_data *p2p = eloop_ctx;
	if (p2p->go_neg_peer == NULL) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "[DEBUG] p2p->go_neg_peer == NULL");
		return;
	}
	p2p->cfg->stop_listen(p2p->cfg->cb_ctx);
	p2p->go_neg_peer->status = P2P_SC_SUCCESS;
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "[DEBUG] p2p_neg_req_wait_rx_resp_timeout :  p2p_connect_send again");
...
int p2p_probe(struct p2p_data *p2p, unsigned int timeout,
	     int freq, const u8 *ssid, size_t ssid_len)
{
	int res;

	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Starting probe %s at %d",
		wpa_ssid_txt(ssid, ssid_len), freq);
	if (p2p->p2p_scan_running) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: p2p_scan is "
			"already running");
	}
	...
	//p2p->find_type = type;
	p2p_device_clear_reported(p2p);
	/* Do not enter search state so we won't probe on other channels */
	p2p_set_state(p2p, P2P_IDLE);
	//p2p_set_state(p2p, P2P_SEARCH);
	eloop_cancel_timeout(p2p_find_timeout, p2p, NULL);
	if (timeout)
		eloop_register_timeout(timeout, 0, p2p_find_timeout,
				       p2p, NULL);
	res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_SPECIFIC, freq, p2p->num_req_dev_types,
						 p2p->req_dev_types, ssid, ssid_len);
		...
		unsigned int force_freq, int persistent_group)
{
	...
	if (p2p->after_scan_tx) {
		 ...
		 */
		wpa_msg(p2p->cfg->msg_ctx, MSG_ERROR, "P2P: Dropped "
...
// ZZZ
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
	"[DEBUG] Cancel the p2p_neg_req_wait_rx_resp_timeout timer");
	eloop_cancel_timeout(p2p_neg_req_wait_rx_resp_timeout, p2p, NULL);
	eloop_register_timeout(1, 0, p2p_neg_req_wait_rx_resp_timeout, p2p, NULL);
...
// ZZZ
static void p2p_rx_p2p_monitor(struct p2p_data *p2p, const u8 *data)
{	
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		"[DEBUG] enter p2p_rx_p2p_monitor");
	switch (data[0]) {
		case P2P_GO_NEG_RESP:
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
				"[DEBUG] Get P2P_GO_NEG_RESP, cancel p2p_neg_req_wait_rx_resp_timeout");
			eloop_cancel_timeout(p2p_neg_req_wait_rx_resp_timeout, p2p, NULL);
			break;
		case P2P_PROV_DISC_RESP:
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
				"[DEBUG] Get P2P_PROV_DISC_RESP, cancel p2p_prov_disc_req_wait_rx_resp_timeout");
			eloop_cancel_timeout(p2p_prov_disc_req_wait_rx_resp_timeout, p2p, NULL);
			break;

		default:
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
			"[DEBUG] Get Nothing, break");
			break;
	}
}
...
static void p2p_sd_cb(struct p2p_data *p2p, int success)
{
	...
	/*for p2p Serv Disc*/
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "@P2P: SD_State %s -> %s",
		p2p_sd_state_txt(p2p->sd_state), p2p_sd_state_txt(SD_REQ_SEND));

	p2p_set_sd_state(p2p, SD_REQ_SEND);
	p2p_sd_set_timeout(p2p, 2, 0);
}
...
static void p2p_sd_state_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Timeout (state=%s)",
		p2p_sd_state_txt(p2p->state));

	/*for p2p Serv Disc*/
	switch (p2p->state) {
	case SD_IDLE:
		p2p_set_sd_state(p2p, SD_IDLE);
		break;
	case SD_REQ_SEND:
		p2p_set_sd_state(p2p, SD_REQ_SEND);
		break;
		    ...
		    size_t len, unsigned int wait_time)
{
	...
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "Send Serv Disc Response, After finish this action Calls send_action_cb call_back function.");
