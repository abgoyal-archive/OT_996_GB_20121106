...
int p2p_start_sd(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	struct p2p_sd_query *query;
	int freq;
	char decline_magic[4] = {0x12, 0x34, 0x56, 0x78};
	...
	if (wpabuf_len(query->tlvs) == 4) {
		if (!os_memcmp(decline_magic,wpabuf_head(query->tlvs),4))
			p2p_sd_cancel_request(p2p, query);
	}
	wpabuf_free(req);
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
			...
			"Get GAS initial Request Packet");
		     ...
		     u8 dialog_token, const struct wpabuf *resp_tlvs)
{
	...
	Build Serv Disc Response include service indormation, then send it back thru p2p_send_action function.
	
	*/
	struct wpabuf *resp;
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "Start to build Serv Disc Response Packet");
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	/*
	Get the GAS initial Response Packer from p2p Devices which receive GAS Request from me.
	*/
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
				"Get GAS initial response packet, start to determine");
	if ((p2p->state != P2P_SD_DURING_FIND&& p2p->sd_state != SD_REQ_SEND) || p2p->sd_peer == NULL ||
    ...
    wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "Set sd_state from SD_REQ_SEND to SD_IDLE");	
	p2p_set_sd_state(p2p,SD_IDLE);
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "Clear the sd time out");	
	p2p_sd_clear_timeout(p2p);
