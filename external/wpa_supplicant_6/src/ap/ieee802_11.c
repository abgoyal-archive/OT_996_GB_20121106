			  ...
			  const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (hapd->num_sta < hapd->conf->max_num_sta) {
		wpa_msg(hapd->msg_ctx_parent, MSG_INFO,
			AP_STA_MAX_NUM "reach=0");
		wpa_printf(MSG_DEBUG, "%s(): parent " AP_STA_MAX_NUM "reach=0", __func__);
	}
#endif
}
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	if ((!hapd->conf->ieee802_1x && !hapd->conf->wpa) ||
		...
		if(hapd->msg_ctx_parent) {
			wpa_msg(hapd->msg_ctx_parent, MSG_INFO,
				AP_STA_CONNECTED MACSTR, MAC2STR(sta->addr));
			wpa_printf(MSG_DEBUG, "%s(): parent " AP_STA_CONNECTED MACSTR, __func__, MAC2STR(sta->addr));
		}
		
		if (hapd->num_sta >= hapd->conf->max_num_sta) {
			wpa_msg(hapd->msg_ctx_parent, MSG_INFO,
				AP_STA_MAX_NUM "reach=1");
			wpa_printf(MSG_DEBUG, "%s(): parent " AP_STA_MAX_NUM "reach=1", __func__);
		}
#endif /* ANDROID_BRCM_P2P_PATCH */
	}
