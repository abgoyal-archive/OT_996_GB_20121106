...
wpa_driver_wext_event_wireless_custom(void *ctx, char *custom)
{
	...
	wpa_printf(MSG_ERROR, "RTM_NEWLINK: Custom wireless event: '%s'",
		   custom);

	os_memset(&data, 0, sizeof(data));

#ifdef ANDROID
	if (os_strncmp(custom, "STOP", 4) == 0) {
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case IWEVCUSTOM:
			if (custom + iwe->u.data.length > end) {
				wpa_printf(MSG_ERROR, "nl80211: Invalid "
					   "IWEVCUSTOM length");
				return;
			}
...
static int wpa_driver_wext_get_ifflags_ifname(struct wpa_driver_nl80211_data *drv,
...
static int wpa_driver_wext_set_ifflags_ifname(struct wpa_driver_nl80211_data *drv,
                                              const char *ifname, int flags)
{
        struct ifreq ifr;
						 ...
						 u8 *buf, size_t len)
{
	...
	while (RTA_OK(attr, attrlen)) {
		if (attr->rta_type == IFLA_IFNAME) {
			...
			wpa_driver_nl80211_event_wireless(drv, ((char *) attr) + rta_len,
			       ...
			       struct nlattr *resp_ie)
{
	...
	{
		wpa_printf(MSG_DEBUG, "%s : Set wpa_s->ap_wapi_ie and wpa_s->ap_wapi_ie_len\n",__func__);
		wpa_s->ap_wapi_ie = drv->ap_wapi_ie;
...
static void wpa_driver_nl80211_check_iface(struct wpa_driver_nl80211_data *drv, struct i802_bss *bss)
{
	int flags;

	if (!drv->driver_is_loaded) {
		wpa_printf(MSG_ERROR, "nl80211: %s should not if up after driver stop",__func__);
		return;
	}

	if ((wpa_driver_wext_get_ifflags_ifname(drv, bss->ifname, &flags) == 0) && !(flags & IFF_UP)) {
		wpa_printf(MSG_ERROR, "nl80211: %s iface is down, change it to up",__func__);
		wpa_driver_wext_set_ifflags_ifname(drv, bss->ifname, flags|IFF_UP);
	}
	else
		wpa_printf(MSG_DEBUG, "nl80211: %s iface is UP",__func__);
}
...
static int wpa_driver_nl80211_set_country(void *priv, const char *alpha2_arg)
{
	...
	wpa_driver_nl80211_check_iface(drv, bss);
...
static int bss_info_handler(struct nl_msg *msg, void *arg)
{
	...
	for (i = 0; i < res->num; i++) {
		...
		wpa_printf(MSG_ERROR, "nl80211: Remove duplicated scan result "
				      ...
				      const u8 *key, size_t key_len)
{
	...
	{
			wpa_printf(MSG_DEBUG, "%s : Set seq && seq_len to NL80211_ATTR_KEY_SEQ\n",__func__);
	...
	struct wpa_driver_associate_params *params)
{
	...
	wpa_printf(MSG_DEBUG, "%s: params->ssid_len=%d\n", __FUNCTION__, params->ssid_len);
#endif
	...
	{
		wpa_printf(MSG_DEBUG, "%s: NL80211_AUTHTYPE_OPEN_SYSTEM\n", __func__);
		type = NL80211_AUTHTYPE_OPEN_SYSTEM;
	}
#endif
	else
...
skip_auth_type:
	if (params->wpa_ie && params->wpa_ie_len) {
		...
		{
			wpa_printf(MSG_DEBUG, "  * IW_AUTH_WAPI_VERSION_1");
	...
	if (params->pairwise_suite != CIPHER_NONE) {
		...
		{
			wpa_printf(MSG_DEBUG, " * Set SUITES_PAIRWISE to WLAN_CIPHER_SUITE_SMS4");
	...
	else
	{
		...
		{
			int cipher;
			wpa_printf(MSG_DEBUG, " * Set SUITE_GROUP to WLAN_CIPHER_SUITE_SMS4");
			cipher = WLAN_CIPHER_SUITE_SMS4;
			params->group_suite = WLAN_CIPHER_SUITE_SMS4;
			NLA_PUT_U32(msg, NL80211_ATTR_CIPHER_SUITE_GROUP, cipher);
		}
#endif
	}
			   ...
			   int reason)
{
	...
	if (!memcmp(addr, b_mac, 6)) {
		wpa_printf(MSG_ERROR, "penguin, ignore broadcast mac!\n");
		return 0;
	}
#endif

	wpa_driver_nl80211_check_iface(drv, bss);
	wpa_printf(MSG_ERROR, "penguin, enter deauth %02X:%02X:%02X:%02X:%02X:%02X!!\n",
		addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);

	memset(set_buf, 0, 256);
	memcpy(set_buf, "DEAUTH_STA", strlen("DEAUTH_STA"));

	memcpy(set_buf + strlen("DEAUTH_STA") + 1, addr, ETH_ALEN);
	//wpa_driver_priv_driver_cmd(priv, "DEAUTH_STA", set_buf, 256);
	wpa_driver_priv_driver_cmd(priv, set_buf, set_buf, 256);
...
static int wpa_driver_priv_driver_cmd(void *priv, char *cmd, char *buf, size_t buf_len) {
	...
	os_strncpy(&ifr.ifr_name[0], bss->ifname, IFNAMSIZ);
	...
	if (os_memcmp(cmd, "SET_AP_WPS_P2P_IE", strlen("SET_AP_WPS_P2P_IE")) != 0)
		wpa_printf(MSG_ERROR, "nl80211: issue private commands %s\n", cmd);
	//PR-231878 Qiwu.Li Modified begin
	if (os_strcasecmp(cmd, "STOP") == 0) {
		if ((wpa_driver_wext_get_ifflags_ifname(drv, drv->first_bss.ifname, &flags) == 0) &&
			(flags & IFF_UP)) {
				wpa_printf(MSG_ERROR, "WEXT-CFG80211: Got driver stop. iface is up, change it to down");
				flags &= ~IFF_UP;
				wpa_driver_wext_set_ifflags_ifname(drv, drv->first_bss.ifname, flags);
		}
		drv->driver_is_loaded = FALSE;
	}

	if ((ret = ioctl(drv->ioctl_sock, SIOCDEVPRIVATE + 1, &ifr)) < 0)
		wpa_printf(MSG_ERROR, "nl80211: failed to issue private commands\n");
	else {
		ret = 0;
		...
		if (os_strcasecmp(cmd, "START") == 0) {
			if ((wpa_driver_wext_get_ifflags_ifname(drv, drv->first_bss.ifname, &flags) == 0) &&
				!(flags & IFF_UP)) {
					wpa_printf(MSG_ERROR, "WEXT-CFG80211: Got driver started. iface is down, change it to up");
					wpa_driver_wext_set_ifflags_ifname(drv, drv->first_bss.ifname, flags|IFF_UP);
			}
			drv->driver_is_loaded = TRUE;
		}
		if (os_memcmp(buf, "SET_AP_WPS_P2P_IE", strlen("SET_AP_WPS_P2P_IE")) != 0)
			wpa_printf(MSG_ERROR, "%s %s len = %d, %d", __func__, buf, ret, strlen(buf));
	}
	//PR-231878 Qiwu.Li Modified end
	return ret;
}
...
static int wpa_driver_get_p2p_noa(void *priv, u8* buf, size_t len) {
	char cmd_buf[200];
	int ret = 0;
	wpa_printf(MSG_DEBUG, "nl80211: %s: Entry", __func__);
	strncpy(cmd_buf, "P2P_GET_NOA", sizeof(cmd_buf));
	ret = wpa_driver_priv_driver_cmd(priv, cmd_buf, (char*)cmd_buf, sizeof(cmd_buf));
	if (ret < 0) {
		wpa_printf(MSG_DEBUG, "nl80211: %s: P2P_GET_NOA returned an error", __func__);
		return 0;
	}
	else {
		hexstr2bin(cmd_buf, buf, os_strlen(cmd_buf)/2);
		return os_strlen(cmd_buf)/2;
	}
}
static int wpa_driver_set_p2p_noa(void *priv, u8 count, int start, int duration) {
	...
	wpa_printf(MSG_DEBUG, "nl80211: %s: Entry", __func__);
	snprintf(buf, sizeof(buf), "%s %d %d %d", "P2P_SET_NOA", count, start, duration);
	return wpa_driver_priv_driver_cmd(priv, buf, buf, strlen(buf)+1);
}
static int wpa_driver_set_p2p_ps(void *priv, int legacy_ps, int opp_ps, int ctwindow) {
	char buf[200];
	memset(buf, 0, sizeof(buf));
	wpa_printf(MSG_DEBUG, "nl80211: %s: Entry", __func__);
	snprintf(buf, sizeof(buf), "%s %d %d %d", "P2P_SET_PS", legacy_ps, opp_ps, ctwindow);
	return wpa_driver_priv_driver_cmd(priv, buf, buf, strlen(buf) + 1);
}
static int
wpa_driver_set_ap_wps_p2p_ie(void *priv, const struct wpabuf *beacon,
	...
	for (i = 0; cmd_arr[i].cmd != -1; i++) {
		...
		*pbuf++ = '\0';
		...
		if (ap_wps_p2p_ie) {
			...
			ret = wpa_driver_priv_driver_cmd(priv, buf, buf,
