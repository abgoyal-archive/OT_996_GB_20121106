				       ...
				       const u8 *key, size_t key_len)
{
	...
	struct iw_encode_ext *ext;
#ifdef WAPI
	wpa_printf(MSG_DEBUG, "%s : Start\n", __func__);
	wpa_printf(MSG_DEBUG, "%s: alg=%d addr=%p key_idx=%d set_tx=%d seq_len=%lu key_len=%lu",
		__func__, alg, addr, key_idx, set_tx, (unsigned long) seq_len, (unsigned long) key_len);
