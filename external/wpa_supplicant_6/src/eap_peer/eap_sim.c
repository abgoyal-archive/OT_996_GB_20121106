...
/*09/05/12 | Qiwu.Li          |                          |Add the realm      */
...
#ifdef ANDROID_JRD_SIM_AKA
			if (conf->pcsc) {
				if (scard_gsm_auth(conf->aid, conf->apptype, data->rand[0],
						   data->sres[0], data->kc[0]) ||
					scard_gsm_auth(conf->aid, conf->apptype, data->rand[1],
						   data->sres[1], data->kc[1]) ||
					(data->num_chal > 2 &&
					 scard_gsm_auth(conf->aid, conf->apptype, data->rand[2],
							data->sres[2], data->kc[2]))) {
					wpa_printf(MSG_DEBUG, "EAP-SIM: GSM SIM "
						   "authentication could not be completed");
					return -1;
...
{
	//PR-297647 Qiwu.li modified begin
	if (attr->next_pseudonym) {
		...
		data->pseudonym[data->pseudonym_len] = '\0';
	}
	//PR-297647 Qiwu.li modified end
...
{
	//PR-264616 Qiwu.Li Modified begin
	wpa_msg(sm->msg_ctx, MSG_INFO, "EAP-SIM: AT_NOTIFICATION %d",attr->notification);
	//PR-264616 Qiwu.Li Modified end
