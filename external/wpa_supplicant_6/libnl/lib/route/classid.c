...
#ifdef ANDROID
/* XXX There are no binary tree implementations in Android bionic */
void *tsearch(const void *key, void **rootp, int(*compar)(const void *, const void *));
void *tfind(const void *key, const void **rootp, int(*compar)(const void *, const void *));
void tdestroy (void *root, void (*free_node)(void *nodep));

static void *nl_array[CLASSID_NAME_HT_SIZ];
static int nmemb = 0;
#endif
...
void *tsearch(const void *key, void **rootp, int(*compar)(const void *, const void *))
{
	...
	if (rootp == NULL)
		return NULL;
	if (nmemb == 0) {
		*rootp = node = &nl_array[nmemb];
		nl_array[nmemb++] = key;
	} else {
		qsort(nl_array, nmemb, sizeof(void *), compar);
		node = bsearch(key, nl_array, nmemb, sizeof(void *), compar);
		if (node == NULL) {
			if (nmemb < CLASSID_NAME_HT_SIZ) {
				node = &nl_array[nmemb];
				nl_array[nmemb++] = key;
			} else {
				return NULL;
