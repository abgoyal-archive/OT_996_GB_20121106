					    ...
					    struct wpa_ssid *group)
{
	...
	for (ssid = group; ssid; ssid = ssid->pnext) {
		...
		if (property_get("ro.wifi.threshold.enable", threshold_enable, NULL) &&
				strcmp(threshold_enable, "1") == 0) {
			if(bss->level < -77) {
		...
		if (wpa && (ssid->key_mgmt == WPA_KEY_MGMT_NONE)) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - WEP enable WPA network not"
				   "allowed");
			continue;
		}
			  ...
			  struct wpa_ssid **selected_ssid)
{
	...
	for (i = 0; i < scan_res->num && !selected; i++) 
	{
		...
		*/
		...
		u8 *wapi_ie = wpa_scan_get_ie(bss, WLAN_EID_WAPI);
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_P2P_DEV_FOUND: {
		 ...
		 */
		wpas_dev_found(wpa_s, data->p2p_dev_found.addr, &peer_info, 1, data->p2p_dev_found.go_dev_addr);
		break;
		}
	case EVENT_P2P_GO_NEG_REQ_RX:
		...
		break;
	case EVENT_P2P_GO_NEG_RES_RX:
		wpas_go_neg_res_rx(wpa_s, data->p2p_go_neg_res_rx.src,
				   data->p2p_go_neg_res_rx.status);
			break;
	...
	case EVENT_INTERFACE_DISABLED:
		...
		wpa_supplicant_mark_disassoc(wpa_s);
		/* XXX Do not set interface disabled state */
