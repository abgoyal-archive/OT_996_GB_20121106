...
/*   Reference documents : TN wpa_supplicant                                 */

/*---------------------------------------------------------------------------*/

/*    Comments :                                                             */

/*    File     :external/wap_supplicant_6                                    */
...
#ifdef CONFIG_WPS
	if (os_strcmp(cmd, "device_name") == 0) {
...
//Qiwu.Li-001 Modified begin
static int wpa_supplicant_ctrl_iface_stop_wps(struct wpa_supplicant *wpa_s)
{
	return wpas_wps_stop_wps(wpa_s);
}
//Qiwu.Li-001 Modified end
...
#ifdef WAPI
		wpa_printf(MSG_DEBUG, "%s , check ssid->wapi ", __FUNCTION__);
					...
					char *cmd, char *buf, size_t buflen)
{
	...
	if (*cmd == '\0') {
		...
		ret = os_snprintf(pos, end-pos, "Current level: %d\n"
			...
			"{0-MSG_EXCESSIVE, 1-MSG_MSGDUMP, 2-MSG_DEBUG, 3-MSG_INFO, 4-MSG_WARNING, 5-MSG_ERROR}\n"
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	if(wpa_s->conf->software_sim)
	{
		if(wpa_config_get(ssid,"eap")!=NULL)
		{
			if( os_strcmp(name,"imsi")==0 && os_strcmp(wpa_config_get(ssid,"eap"),"SIM")==0 )
			{
				...
				wpa_printf(MSG_DEBUG,"Soft test of SIM, IMSI: %s\n",str);
				value = str;
			}
			if( os_strcmp(name,"imsi")==0 && os_strcmp(wpa_config_get(ssid,"eap"),"AKA")==0 )
			{
				...
				wpa_printf(MSG_DEBUG,"Soft test of AKA, IMSI: %s\n",str);
				value = str;
			}
		}
	}
	...
	//PR-224185 Qiwu.Li modified begin
	wpa_blacklist_clear(wpa_s);
	//PR-224185 Qiwu.Li modified end
...
static int p2p_ctrl_reject_invite(struct wpa_supplicant *wpa_s, char *cmd,
				  char *buf, size_t buflen)
{
	u64 ref;
	int res;
	...
	if (tlvs == NULL)
		return -1;
	if (hexstr2bin(decline_magic, wpabuf_put(tlvs, len), len) < 0) {
		wpabuf_free(tlvs);
		return -1;
	}
	ref = (unsigned long) wpas_p2p_sd_request(wpa_s, dst, tlvs);
	...
	if (res < 0 || (unsigned) res >= buflen)
		return -1;
	return wpas_p2p_reject(wpa_s, dst);
}
...
static int p2p_ctrl_invite_group(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*pos = '\0';
	...
	//PR189379-Qiao.Li-001-begin
       else if (os_memcmp(wpa_s->bssid,"\x00\x00\x00\x00\x00\x00", ETH_ALEN) != 0){
	   wpa_printf(MSG_ERROR, "set go_dev_addr = " MACSTR, MAC2STR(wpa_s->bssid));
           os_memcpy(go_dev_addr,wpa_s->bssid,ETH_ALEN);
           go_dev=go_dev_addr;
       }
	//PR189379-Qiao.Li-001-end
...
static int p2p_ctrl_disconnect(struct wpa_supplicant *wpa_s, char *cmd)
{
	struct wpa_supplicant *iface;
	char *pos, *ifname;
	u8 addr[ETH_ALEN];
	...
	if (os_strncmp(cmd, "group=", 6) != 0) {
		wpa_printf(MSG_DEBUG, "CTRL: Invalid P2P_DISCONNECT parameters '%s'",
			   cmd);
		return -1;
	}
	...
	*pos = '\0';
	...
	if (iface->ap_iface == NULL){
		wpa_printf(MSG_DEBUG, "P2P: Interface '%s' is not a P2P interface", ifname);
		return -1;
	}
	...
	return wpa_supplicant_p2p_disconnect(iface, addr);
}


static int p2p_ctrl_member(struct wpa_supplicant *wpa_s, char *cmd,
			 char *buf, size_t buflen)
{
	struct wpa_supplicant *iface;
	char *pos, *ifname;
	...
	if (os_strncmp(cmd, "group=", 6) != 0) {
		wpa_printf(MSG_DEBUG, "CTRL: Invalid P2P_MEMBER parameters '%s'",
			   cmd);
		return -1;
	}
	...
	*pos = '\0';
	...
	if (os_strcmp(pos, "FIRST") == 0) {
		addr_ptr = NULL;
		next = 0;
	} else if (os_strncmp(pos, "NEXT-", 5) == 0) {
		if (hwaddr_aton(pos + 5, addr) < 0)
			return -1;
		addr_ptr = addr;
		next = 1;
	} else {
	...
	return wpa_supplicant_p2p_get_member_info(iface, addr_ptr, next,
				 buf, buflen);
}

//PR200165-Qiao.Li-001 Added begin
#ifdef WPA_GET_GC_LIST
static int p2p_ctrl_gcmembers(struct wpa_supplicant *wpa_s, char *cmd,
			 char *buf, size_t buflen)
{
	char *ifname;
	char *pos, *end, *pos2;
	char addr[ETH_ALEN * 3];
	char cmd2[128], buf2[4096];
	int res;
	char iface_name[16];
	struct wpa_supplicant *iface;
	int iface_len = 0;
	...
	if (os_strncmp(cmd, "group=", 6) != 0) {
		wpa_printf(MSG_ERROR, "%s(): Invalid P2P_MEMBERS parameters '%s'",
			   __func__, cmd);
		return -1;
	}

	ifname = cmd + 6;
	iface_len = os_strlen(ifname);
	os_memset(iface_name, 0, 16);
	os_memcpy(iface_name, ifname, iface_len > 16? 16: iface_len);
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
		if (os_strcmp(iface->ifname, iface_name) == 0)
			break;
	}
	if (iface != NULL) {
		wpa_printf(MSG_DEBUG, "%s(): got iface = %s, len=%d", __func__, iface_name, iface_len);
		wpas_p2p_get_members_info((void*)wpa_s, iface);
	}
	return 0;
}
#endif
//PR200165-Qiao.Li-001 Added end
static int p2p_ctrl_members(struct wpa_supplicant *wpa_s, char *cmd,
			 char *buf, size_t buflen)
{
	char *ifname;
	char *pos, *end, *pos2;
	...
	ifname = cmd + 6;
	res = os_snprintf(cmd2, sizeof(cmd2), "group=%s FIRST", ifname);
	if (res < 0 || (size_t) res >= sizeof(cmd2))
		return -1;
	cmd2[sizeof(cmd2) - 1] = '\0';

	if ((res = p2p_ctrl_member(wpa_s, cmd2, buf2, sizeof(buf2))) == -1) {
		return -1;
	}
	...
	do {
		wpa_printf(MSG_DEBUG, "%s(): res= %d, buf2 '%s'", __func__, res, buf2);

		pos2 = buf2;
		while (*pos2 != '\0' && *pos2 != '\n')
			pos2++;
		*pos2++ = '\0';
		os_strlcpy(addr, buf2, sizeof(addr));
		...
		res = os_snprintf(cmd2, sizeof(cmd2), "group=%s NEXT-%s", ifname, addr);
		if (res < 0 || (size_t) res >= sizeof(cmd2))
			return -1;
		cmd2[sizeof(cmd2) - 1] = '\0';
	} while ((res = p2p_ctrl_member(wpa_s, cmd2, buf2, sizeof(buf2))) != -1);

	wpa_printf(MSG_DEBUG, "%s(): %d, buf '%s'", __func__, pos - buf, buf);
	return pos - buf;
}


static int p2p_ctrl_probe(struct wpa_supplicant *wpa_s, char *cmd)
{
	struct wpa_supplicant *iface;
	char *pos, *ifname;
	u8 addr[ETH_ALEN];
	...
	if (os_strncmp(cmd, "group=", 6) != 0) {
		wpa_printf(MSG_DEBUG, "CTRL: Invalid P2P_PROBE parameters '%s'",
			   cmd);
		return -1;
	}
...
static int p2p_ctrl_peers(struct wpa_supplicant *wpa_s,
			 char *buf, size_t buflen)
{
	char *pos, *end, *pos2;
	char addr[ETH_ALEN * 3];
	char cmd[128], buf2[4096];
	int res;

	res = os_snprintf(cmd, sizeof(cmd), "FIRST");
	if (res < 0 || (size_t) res >= sizeof(cmd))
		return -1;
	cmd[sizeof(cmd) - 1] = '\0';

	if ((res = p2p_ctrl_peer(wpa_s, cmd, buf2, sizeof(buf2))) == -1) {
		return -1;
	}
	...
	do {
		...
		*pos2++ = '\0';
		...
		res = os_snprintf(cmd, sizeof(cmd), "NEXT-%s", addr);
		if (res < 0 || (size_t) res >= sizeof(cmd))
			return -1;
		cmd[sizeof(cmd) - 1] = '\0';
	} while ((res = p2p_ctrl_peer(wpa_s, cmd, buf2, sizeof(buf2))) != -1);
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (res == -2) {
			...
			reply_len = -1;
	//Qiwu.Li-001 Modified begin
	}else if (os_strcmp(buf, "STOP_WPS") == 0) {
	...
	//Qiwu.Li-001 Modified end
		...
		reply_len = p2p_ctrl_serv_disc_req(wpa_s, buf + 18, reply,
						   reply_size);
	} else if (os_strncmp(buf, "P2P_REJECT_INVITE ", 18) == 0) {
		reply_len = p2p_ctrl_reject_invite(wpa_s, buf + 18, reply,
						   reply_size);
		...
		if (p2p_ctrl_disconnect(wpa_s, buf + 15) < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "P2P_MEMBERS ", 12) == 0) {
		reply_len = p2p_ctrl_members(wpa_s, buf + 12, reply,
						  reply_size);
//PR200165-Qiao.Li-002 Added begin
#ifdef WPA_GET_GC_LIST
	} else if (os_strncmp(buf, "P2P_GCMEMBERS ", 14) == 0) {
		reply_len = p2p_ctrl_gcmembers(wpa_s, buf + 14, reply,
						  reply_size);
#endif
//PR200165-Qiao.Li-002 Added end
	} else if (os_strncmp(buf, "P2P_MEMBER ", 11) == 0) {
		reply_len = p2p_ctrl_member(wpa_s, buf + 11, reply,
						  reply_size);
	} else if (os_strncmp(buf, "P2P_PEERS", 9) == 0) {
		reply_len = p2p_ctrl_peers(wpa_s, reply,
						  reply_size);
	} else if (os_strncmp(buf, "P2P_PEER ", 9) == 0) {
		reply_len = p2p_ctrl_peer(wpa_s, buf + 9, reply,
					      reply_size);
	} else if (os_strncmp(buf, "P2P_PROBE ", 10) == 0) {
		if (p2p_ctrl_probe(wpa_s, buf + 10) < 0)
			reply_len = -1;
	...
	*(reply + reply_len) = '\0';
	wpa_printf(MSG_DEBUG, "REPLY: %s", reply);
	return reply;
}
