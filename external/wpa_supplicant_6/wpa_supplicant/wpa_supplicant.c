			      ...
			      enum wpa_states state)
{
	...
	wpa_dbg(wpa_s, MSG_ERROR, "State: %s -> %s",
...
/*#ifdef ANDROID
	...
	if (wpa_s->wpa_state != old_state) {
		wpas_notify_state_changed(wpa_s, /*wpa_s->wpa_state*/reported_state, old_state);//harry
...
/*
		config->group_cipher = WLAN_CIPHER_SUITE_SMS4;
		config->pairwise_cipher = WLAN_CIPHER_SUITE_SMS4;
*/
#endif
	}
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
...
#ifdef CONFIG_P2P
		wpa_printf(MSG_ERROR, "%s: call wpas_p2p_set_freq(), freq=%d\n", __FUNCTION__, bss->freq);
		wpas_p2p_set_freq(wpa_s, bss->freq);
#endif
#endif	
	...
	if ( (ssid->wapi) || (ssid->proto & WPA_PROTO_WAPI) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_CERT ) ) { /* associating to a WAPI network */
		...
		wpa_printf(MSG_DEBUG, "## WAPI Info : ssid->wapi %d ssid->proto %d ssid->key_mgmt %d @@", ssid->wapi, ssid->proto, ssid->key_mgmt);
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (ssid == NULL) {
		...
		ssid->disabled = 0;
		/*???Void the STA try to connect the AP automatically by SIM_AKA*/
		/*if(wpa_config_get(ssid,"eap")!=NULL) {
			if( os_strcmp(wpa_config_get(ssid,"eap"),"SIM")==0 || os_strcmp(wpa_config_get(ssid,"eap"),"AKA")==0 )
			{
				ssid->disabled = 1;
			}
		}*/
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
...
#ifdef WAPI
		wpa_printf(MSG_DEBUG, "[terry trace] %s : wpa_s->driver->send_eapol ",__FUNCTION__);
		wpa_s->l2_wapi = l2_packet_init(wpa_s->ifname,
						wpa_drv_get_mac_addr(wpa_s),
						ETH_P_WAI,
						wapi_asue_rx_wai, wpa_s, 0);
		wpa_printf(MSG_DEBUG, "wpa_s->l2_wapi = %p ", wpa_s->l2_wapi);
		wpa_printf(MSG_DEBUG, "[terry trace]wpa_s->l2_wapi = %p ", wpa_s->l2_wapi);
#endif
	...
	if (wpa_s->l2_wapi && l2_packet_get_own_addr(wpa_s->l2_wapi, wpa_s->wapi_own_addr)) {
			wpa_printf(MSG_DEBUG, "[terry trace] %s : get own wapi L2 address failed\n ",__FUNCTION__);
		wpa_printf(MSG_ERROR, "Failed to get own WAPI L2 address");
		return -1;
	}
	wpa_printf(MSG_DEBUG, "[terry trace] %s ",__FUNCTION__);
