...
static int wpas_p2p_scan(void *ctx, enum p2p_scan_type type, int freq,
			 ...
			 const u8 *req_dev_types, const u8 *ssid, size_t ssid_len)
{
...
static void wpas_p2p_group_delete(struct wpa_supplicant *wpa_s)
{
	...
	case P2P_GROUP_REMOVAL_UNAVAILABLE:
		...
		break;
	case P2P_GROUP_REMOVAL_DEAUTH:
		reason = " reason=DEAUTH";
		break;
	default:
		reason = "";
		break;
...
void wpas_go_neg_res_rx(void *ctx, const u8 *src, int status)
{
	struct wpa_supplicant *wpa_s = ctx;
	wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_RESPONSE MACSTR
...
void wpas_dev_found(void *ctx, const u8 *addr,
		    ...
		    int new_device, const u8 *go_dev_addr)
{
	struct wpa_supplicant *wpa_s = ctx;
	...
	if (go_dev_addr == NULL) {
		...
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR
			" p2p_dev_addr=" MACSTR
			" pri_dev_type=%s name='%s' config_methods=0x%x "
			"dev_capab=0x%x group_capab=0x%x go_dev_addr=" MACSTR,
			MAC2STR(addr), MAC2STR(info->p2p_device_addr),
			wps_dev_type_bin2str(info->pri_dev_type, devtype,
					     sizeof(devtype)),
			info->device_name, info->config_methods,
			info->dev_capab, info->group_capab, MAC2STR(go_dev_addr));
	}
			     ...
			     const struct wpabuf *probe_resp_ie)
{
	...
	struct wpa_supplicant *tmp_wpa_s = NULL;
	struct wpa_interface iface;
	struct wpa_supplicant *group_wpa_s;
	u8 go_dev_addr[ETH_ALEN];
	u8 dev_addr[ETH_ALEN];
	u8 bssid[ETH_ALEN];
	int p2p_rule = 0; /*0 : Standalone device, 1 : GC, 2 : GO*/
	...
	if(tmp_wpa_s != NULL) {
		struct wpa_ssid *ssid = tmp_wpa_s->current_ssid;
		if(ssid != NULL) {
			switch (ssid->mode) {
		...
		if(tmp_wpa_s->p2p_group_interface == P2P_GROUP_INTERFACE_GO) {
			wpa_printf(MSG_ERROR, "p2p_group_interface is P2P_GROUP_INTERFACE_GO");
			p2p_rule = 2;
		}
	}
	if(p2p_rule == 2) {
		     ...
		     u16 update_indic, const u8 *tlvs, size_t tlvs_len)
{
	...
	if (buf) {
		...
		if (!os_strcmp(buf, decline_magic)) {
			wpa_msg_ctrl(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_REJECT
				     MACSTR,
				     MAC2STR(sa));
			os_free(buf);
			return;
		...
		wpa_msg_ctrl(wpa_s, MSG_INFO, P2P_EVENT_SERV_DISC_REQ "%d "
			     ...
			     buf);
#else
		wpa_msg_ctrl(wpa_s, MSG_INFO, P2P_EVENT_SERV_DISC_REQ " p2p_dev_addr=" MACSTR " freq=%d diag_token=%u update_indic=%u tlvs=%s",
					MAC2STR(sa), freq, dialog_token, update_indic, buf);
#endif
		os_free(buf);
	}
		      ...
		      const u8 *tlvs, size_t tlvs_len)
{
	...
	size_t buf_len;
	char *buf;
	int bonjour = 0, upnp = 0, bonjour_status = 0, upnp_status = 0;
						...
						wpa_msg_ctrl(wpa_s, MSG_INFO,
								 ...
								 MAC2STR(sa), update_indic, buf);
									...
									while (pos < end) {
											...
											status = *pos++;
											pos = tlv_end;
											if(srv_proto == 1 ) // bonjour
											{
													bonjour = 1;
													bonjour_status = status;
											}
											else if(srv_proto == 2)
											{
													upnp = 1;
...
#if 0
									wpa_msg_ctrl(wpa_s, MSG_INFO, P2P_EVENT_SERV_DISC_RESP " p2p_dev_addr=" MACSTR " update_indic=%u tlvs=%s bonjour=%u bonjour_status=%u upnp=%u upnp_status=%u",MAC2STR(sa), update_indic, buf, bonjour, bonjour_status, upnp, upnp_status);
											os_free(buf);
#else
							wpa_msg_ctrl(wpa_s, MSG_INFO, P2P_EVENT_SERV_DISC_RESP " p2p_dev_addr=" MACSTR " update_indic=%u bonjour=%u bonjour_status=%u upnp=%u upnp_status=%u",MAC2STR(sa), update_indic, bonjour, bonjour_status, upnp, upnp_status);
											os_free(buf);
#endif
...
static void wpas_invitation_received(void *ctx, const u8 *sa, const u8 *bssid,
				     ...
				     int op_freq, const u8 *sa_dev_name, const u8 *go_dev_name)
{
	struct wpa_supplicant *wpa_s = ctx;
	struct wpa_ssid *s;
	...
	if (!s) {
		if (bssid) {
			wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED
				...
				" bssid=" MACSTR "sa_dev_name=%s"" go_dev_name=%s"" unknown-network",
				MAC2STR(sa), MAC2STR(go_dev_addr),
				MAC2STR(bssid), sa_dev_name?sa_dev_name:"N/A", go_dev_name?go_dev_name:"N/A");
		} else {
			wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED
				"sa=" MACSTR " go_dev_addr=" MACSTR
				"dev_name=%s"" unknown-network",
				MAC2STR(sa), MAC2STR(go_dev_addr), go_dev_name?go_dev_name:"N/A");
		}
		return;
	}

	wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR
		" persistent=%d" " sa_dev_name=%s"" go_dev_name=%s", MAC2STR(sa), s->id, sa_dev_name?sa_dev_name:"N/A", go_dev_name?go_dev_name:"N/A");
}
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	p2p.go_neg_res_rx = wpas_go_neg_res_rx;
...
#ifdef ANDROID_BRCM_GOSTA_CONCURRENT
		if (freq == 0) {
			struct p2p_data *p2p = wpa_s->global->p2p;
	
			wpa_printf(MSG_ERROR, "P2P: %s: force freq=2 best_24_freq=%d\n", __func__, wpa_s->best_24_freq);
	
			if ( p2p->best_freq_24 ) {
				freq = 2;
				wpa_s->best_24_freq = p2p->best_freq_24;
			} 
	
			if ( p2p->best_freq_5 ) {
				freq = 5;
				wpa_s->best_5_freq = p2p->best_freq_5;
			}
		}
#endif
...
void wpas_p2p_group_remove_notif(struct wpa_supplicant *wpa_s, u16 reason_code)
{
	...
	if((wpa_s != wpa_s->parent) && (wpa_s->p2p_group_interface == P2P_GROUP_INTERFACE_CLIENT) && (wpa_s->key_mgmt != WPA_KEY_MGMT_WPS)) {
		...
		wpa_s->removal_reason = P2P_GROUP_REMOVAL_DEAUTH;
		wpas_p2p_group_delete(wpa_s);
	}
}
#endif
...
int wpas_p2p_cancel(struct wpa_supplicant *wpa_s)
{
...
#if 1 /* add for cancel WPS when GC join GO by Xinrui */
	wpa_printf(MSG_DEBUG, "check ap_iface\n");
	{
		int i = 0;
		struct wpa_supplicant *iface1;
		wpa_printf(MSG_DEBUG, "wpas_wps_cancel\n");
		for(iface1 = wpa_s->global->ifaces; iface1; iface1 = iface1->next) {
			wpa_printf("MSG_DEBUG, iface=%p, ap_iface= %p\n", iface1, iface1->ap_iface);
			i = wpas_wps_cancel(iface1);
		}
		found = found || i;
	}
#endif
	eloop_cancel_timeout(p2p_prov_disc_req_wait_rx_resp_timeout, global->p2p, NULL);
	if (wpa_s->pending_interface_name[0] &&
	    !is_zero_ether_addr(wpa_s->pending_interface_addr))
		found = 1;
	...
	for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		wpa_printf(MSG_DEBUG, "P2P: provisioning=%d, interface_type=%x ",
				wpa_s->p2p_in_provisioning , wpa_s->parent->pending_interface_type );
		if (wpa_s == global->p2p_group_formation &&
		    (wpa_s->p2p_in_provisioning ||
...
#ifdef ANDROID_BRCM_GOSTA_CONCURRENT
int wpas_p2p_set_freq(struct wpa_supplicant *wpa_s, unsigned int freq)
{
	struct p2p_data *p2p = wpa_s->global->p2p;
	u8 op_reg_class, op_channel;

	if ( !p2p ) {
		wpa_printf(MSG_ERROR, "P2P: %s() return due to p2p is null.\n", __FUNCTION__);
		return -1;
	}

	p2p_freq_to_channel(p2p->cfg->country, freq, &op_reg_class, &op_channel);

	wpa_printf(MSG_ERROR, "P2P: set freq = %d, channel = %d, op_reg_class=%d, op_channel=%d\n", freq, op_channel, op_reg_class, op_channel);

	p2p->op_reg_class = op_reg_class;
	p2p->op_channel = op_channel;

	if ( op_channel > 14 ) {
		p2p->best_freq_5 = freq;
	} else {
		p2p->best_freq_24 = freq;
	}
...
int wpas_p2p_probe(struct wpa_supplicant *wpa_s)
{
	struct wpa_ssid *ssid = wpa_s->current_ssid;
	const char *ssid_txt;
	...
	if (ssid != NULL) {
		ssid_txt = wpa_ssid_txt(ssid->ssid, ssid->ssid_len);
		wpa_printf(MSG_DEBUG, "%s(): ssid= %s", __func__, ssid_txt);
	} else {
		return -1;
	}
	if (wpa_s->assoc_freq != 0) {
		wpa_printf(MSG_DEBUG, "%s(): wpa_s->assoc_freq= %d", __func__, wpa_s->assoc_freq);
	} else {
		return -1;
	}

	return p2p_probe(wpa_s->global->p2p, 3, wpa_s->assoc_freq, ssid->ssid, ssid->ssid_len);
}

//PR200165-Qiao.Li-001 Added begin
int wpas_p2p_get_members_info(void *wpa_s, struct wpa_supplicant *iface)
{
	wpa_printf(MSG_DEBUG, "%s(): enter", __func__);
	if ((iface->ap_iface == NULL) ||
	    (iface->ap_iface->bss == NULL) ||
	    (iface->ap_iface->bss[0] == NULL) ||
	    (iface->ap_iface->bss[0]->p2p_group == NULL)) {
		return -1;
	}

	wpa_printf(MSG_DEBUG, "get member start clients member\n");
	return p2p_group_get_clients((struct wpa_supplicant *)wpa_s, iface->ap_iface->bss[0]->p2p_group);
}
//PR200165-Qiao.Li-001 Added end