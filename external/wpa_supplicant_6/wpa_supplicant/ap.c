...
int wpa_supplicant_p2p_disconnect(struct wpa_supplicant *wpa_s, const u8 *addr)
{
	u8 *interface_addr;

	wpa_printf(MSG_DEBUG, "%s(): enter", __func__);
	if ((wpa_s->ap_iface == NULL) ||
	    (wpa_s->ap_iface->bss == NULL) ||
	    (wpa_s->ap_iface->bss[0] == NULL) ||
	    (wpa_s->ap_iface->bss[0]->p2p_group == NULL)) {
		return -1;
	}

	interface_addr = p2p_group_get_client_interface_addr(wpa_s->ap_iface->bss[0]->p2p_group, addr);
	if (interface_addr == NULL) {
		wpa_printf(MSG_DEBUG, "%s(): unknown client " MACSTR, __func__, MAC2STR(addr));
		return -1;
	}
	
	hostapd_drv_sta_deauth(wpa_s->ap_iface->bss[0], interface_addr, WLAN_REASON_DISASSOC_AP_BUSY);
	hostapd_notif_disassoc(wpa_s->ap_iface->bss[0], interface_addr);

	return 0;
}


int wpa_supplicant_p2p_get_member_info(struct wpa_supplicant *wpa_s, const u8 *addr, int next,
		      char *buf, size_t buflen)
{
	wpa_printf(MSG_DEBUG, "%s(): enter", __func__);
	if ((wpa_s->ap_iface == NULL) ||
	...
	return p2p_get_client_info(wpa_s->ap_iface->bss[0]->p2p_group, addr, next, buf, buflen);
}
